...
void PairMAGMEAM::compute_single_pair(int ii, double fmi[3], double fmdsi[6])
{
  ...
  int i,ix,j,n,inum_half,inum_full,errorflag;
  int *ilist_half,*ilist_full,*numneigh_half,**firstneigh_half;
  int *numneigh_full,**firstneigh_full;
  double delij[3], rij2;
  ...
  for (i = 0; i < inum_full; i++) {
    ix = ilist_full[i];
    delij[0] = x[j][0] - x[ix][0];
    delij[1] = x[j][1] - x[ix][1];
    delij[2] = x[j][2] - x[ix][2];
  ...
  for (i = 0; i < inum_full; i++) {
    ...
    meam_inst->update_dens_final(ix,ntype,type,map,scale,errorflag,rij2);
  }
  if (errorflag)
    error->one(FLERR,"MEAM library error {}",errorflag);
  ...
  meam_inst->meam_spin(j,ntype,type,map,scale,x,sp,numneigh_full[j],
		       firstneigh_full[j],offinv[j],fmi,fmdsi,hbar);
  fmi[0] = fmi[0] / hbar;
  fmi[1] = fmi[1] / hbar;
  fmi[2] = fmi[2] / hbar;
