...
MAGMEAM::meam_dens_update(int i, int ntype, int* type, int* fmap, double** x, double** sp,
                     int numneigh, int* firstneigh,
			  int numneigh_full, int* firstneigh_full, int fnoffset, double rij2)
{
  //     Calculate intermediate density terms to be communicated
  if (rij2 < this->cutforcesq) update_rho1(i, ntype, type, fmap, x, sp, numneigh_full, firstneigh_full, &scrfcn[fnoffset], &scrfcnmag[fnoffset], &fcpair[fnoffset]);
